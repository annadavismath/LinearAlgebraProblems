\documentclass{ximera}
\input{../preamble.tex}
\author{Anna Davis}
\license{Creative Commons 4.0 By-NC-SA}
%\outcome{Compute an antiderivative using basic formulas}
\begin{document}
\begin{exercise}
Let $$M=\begin{bmatrix}1 & 0\\0 & 2\end{bmatrix}\begin{bmatrix}0 & -1\\1 & 0\end{bmatrix}$$
Consider the linear transformation $T_M:\RR^2\rightarrow\RR^2$ induced by $M$.  What is the image of the photograph below under $T_M$?  Select the correct image from the four choices below.


\begin{center}   
		\begin{tikzpicture}[scale=1.3]
  \draw[black, thin, fill=yellow,opacity=0.25] (-5,-1) rectangle (5, 5.5);
  \node[] at (2, 4.5)   (c) {Original Photo};
  \node[inner sep=0pt, anchor=south west] (jay) at (0,0)
  {\includegraphics[width=40mm]{blueJay.JPG}};
  \draw[<->] (-4,0)--(4,0);
  \draw[<->] (0,-0.5)--(0,5);
       \end{tikzpicture}
\end{center}

\begin{center}
\begin{tikzpicture}[scale=1.3]
\node[inner sep=0pt, anchor=south east] (jay) at (0,0)
  {\includegraphics[height=80mm]{blueJayA.jpg}};
  \node[] at (-2, 6.5)   (c) {Image A};
  \draw[<->] (-5,0)--(5,0);
  \draw[<->] (0,-0.5)--(0,8);
       \end{tikzpicture}\quad
\begin{tikzpicture}[scale=1.3]
\node[inner sep=0pt, anchor=north east] (jay) at (0,0)
  {\includegraphics[width=80mm]{blueJayB.jpg}};
  \node[] at (-3, 0.5)   (c) {Image B};
  \draw[<->] (-7,0)--(3,0);
  \draw[<->] (0,-3)--(0,1);
       \end{tikzpicture}   

\begin{tikzpicture}[scale=1.3]
\node[inner sep=0pt, anchor=south east] (jay) at (0,0)
  {\includegraphics[width=80mm]{blueJayC.jpg}};
  \node[] at (-3, 2.8)   (c) {Image C};
  \draw[<->] (-7,0)--(3,0);
  \draw[<->] (0,-0.5)--(0,3);
       \end{tikzpicture}\quad
\begin{tikzpicture}[scale=1.3]
\node[inner sep=0pt, anchor=south west] (jay) at (0,0)
  {\includegraphics[width=80mm]{blueJayD.jpg}};
  \node[] at (3, 4.5)   (c) {Image D};
  \draw[<->] (-3,0)--(7,0);
  \draw[<->] (0,-0.5)--(0,6);
       \end{tikzpicture}          
\end{center}

\begin{multipleChoice}
    \choice[correct]{A}
    \choice{B}
    \choice{C}
    \choice{D}
\end{multipleChoice}

 \end{exercise}

 \subsection*{Photo Credit}
[Davis] Erik Davis, CC-BY 4.0.
\end{document}